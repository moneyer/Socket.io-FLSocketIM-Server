...
//        }
        ...
        socket.on('videoChat', function (data, callback) {
            ...
            if (callback) { // 将房间号发送给聊天的双方
                ...
                if (data.chat_type == 0) { // 单聊
                    var targetSocket = vfglobal.socket_Map[data.to_user];
                    if (targetSocket) {
                        targetSocket.emit('videoChat', {"from_user":data.from_user, "room":room, "chat_type":data.chat_type}, timeoutCallback(function (timeout, data) {   //用其自身连接给自己发消息
                    ...
                    io.emit('videoChat', {"from_user":data.from_user, "room":room, "chat_type":data.chat_type});
                }
